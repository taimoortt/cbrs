...
UeLtePhy::UeLtePhy() {
  ...
  Simulator::Init()->Schedule(0.001, &UeLtePhy::SetTxSignalForReferenceSymbols,
                              this);
}

UeLtePhy::~UeLtePhy() { Destroy(); }
...
void UeLtePhy::StartTx(PacketBurst *p) {
#ifdef TEST_DEVICE_ON_CHANNEL
  std::cout << "Node " << GetDevice()->GetIDNetworkNode() << " starts phy tx"
            << std::endl;
#endif
...
void UeLtePhy::StartRx(PacketBurst *p, TransmittedSignal *txSignal) {
#ifdef TEST_DEVICE_ON_CHANNEL
  ...
  for (auto it = rsrp_interference.begin(); it != rsrp_interference.end();
       ++it) {
  ...
  for (int i = 0; i < rsrp_interference.size(); i++) {
    real_rsrp_interference[i] =
        vector<double>(rxSignalValues.size(), rsrp_interference[i]);
  }
#endif
  ...
  double noise_interference =
      10. * log10(pow(10., NOISE / 10) + tot_interference_watt);
  ...
  assert(m_measuredSinr.size() == rxSignalValues.size());
  RSRPReport rsrp_report(rxSignalValues, real_rsrp_interference, NOISE,
  ...
  std::cout << Simulator::Init()->Now() << " UE(" << ue->GetIDNetworkNode()
            ...
            << " interfere_noise(db) " << noise_interference << " sinr(db) "
            << avg_rsrp - noise_interference << " snr(db) " << avg_rsrp - NOISE
            << std::endl;
#endif
  ...
  if (GetErrorModel() != NULL && m_channelsForRx.size() > 0) {
    ...
    phyError = GetErrorModel()->CheckForPhysicalError(m_channelsForRx, cqi_,
                                                      m_measuredSinr);

    if (_PHY_TRACING_) {
      if (phyError) {
        std::cout << "**** YES PHY ERROR (node "
                  << GetDevice()->GetIDNetworkNode() << ") ****" << std::endl;
      } else {
        std::cout << "**** NO PHY ERROR (node "
                  << GetDevice()->GetIDNetworkNode() << ") ****" << std::endl;
      }
    }
  } else {
  ...
  int TBS_ = GetDevice()
                 ...
                 ->GetAmcModule()
                 ->GetTBSizeFromMCS(
                     MCS_, GetBandwidthManager()->GetDlSubChannels().size());
  std::cout << "DL_SINR " << GetDevice()->GetIDNetworkNode() << " "
            ...
            << GetDevice()
                   ...
                   ->GetAbsolutePosition()
                   ->GetCoordinateY()
            << " " << effective_sinr << " " << MCS_ << " " << TBS_ << std::endl;
#endif
...
void UeLtePhy::CreateCqiFeedbacks(std::vector<SinrReport> record) {
  UserEquipment *thisNode = (UserEquipment *)GetDevice();
  if (thisNode->GetCqiManager()->NeedToSendFeedbacks()) {
...
void UeLtePhy::CreateCqiFeedbacks(RSRPReport record) {
  UserEquipment *thisNode = (UserEquipment *)GetDevice();
  if (thisNode->GetCqiManager()->NeedToSendFeedbacks()) {
    thisNode->GetCqiManager()->CreateCqiFeedbacks(record);
  }
...
void UeLtePhy::SendIdealControlMessage(IdealControlMessage *msg) {
  ...
  std::cout << "SendIdealControlMessage (PHY) from  "
...
void UeLtePhy::ReceiveIdealControlMessage(IdealControlMessage *msg) {
  if (msg->GetMessageType() == IdealControlMessage::ALLOCATION_MAP) {
    // std::cout << "ReceiveIdealControlMessage, node " <<
    // GetDevice()->GetIDNetworkNode() << std::endl;
    ...
    if (m_channelsForTx.size() > 0) {
      ...
      UeMacEntity *mac =
          (UeMacEntity *)GetDevice()->GetProtocolStack()->GetMacEntity();
      mac->ScheduleUplinkTransmission(m_channelsForTx.size(),
                                      m_mcsIndexForTx.at(0));
    }
  }
}

void UeLtePhy::SetTxSignalForReferenceSymbols(void) {
  BandwidthManager *s = GetBandwidthManager();
  std::vector<double> channels = s->GetUlSubChannels();
...
TransmittedSignal *UeLtePhy::GetTxSignalForReferenceSymbols(void) {
  return m_txSignalForRerferenceSymbols;
}

void UeLtePhy::SendReferenceSymbols(void) {
  UserEquipment *ue = (UserEquipment *)GetDevice();
