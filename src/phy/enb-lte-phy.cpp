...
void EnbLtePhy::StartTx(PacketBurst *p) {
  // std::cout << "Node " << GetDevice()->GetIDNetworkNode () << " starts phy
  // tx" << std::endl;
  ...
  if (GetErrorModel() != NULL)
    {
          ...
          phyError = GetErrorModel ()->CheckForPhysicalError (channelsForRx,
  cqi, measuredSinr); if (_PHY_TRACING_)
            {
              if (phyError)
                {
                      std::cout << "**** YES PHY ERROR (node " << GetDevice
  ()->GetIDNetworkNode () << ") ****" << std::endl;
                }
              else
                {
                      std::cout << "**** NO PHY ERROR (node " << GetDevice
  ()->GetIDNetworkNode () << ") ****" << std::endl;
                }
            }
    }
...
void EnbLtePhy::SendIdealControlMessage(IdealControlMessage *msg) {
  if (msg->GetMessageType() == IdealControlMessage::ALLOCATION_MAP) {
    ...
    for (it = registeredUe->begin(); it != registeredUe->end(); it++) {
      // std::cout << "SendIdealControlMessage to " << (*it)->GetUE
      // ()->GetIDNetworkNode() << std::endl;
...
void EnbLtePhy::ReceiveIdealControlMessage(IdealControlMessage *msg) {
  ...
  std::cout << "ReceiveIdealControlMessage (PHY) from  "
            ...
            << msg->GetDestinationDevice()->GetIDNetworkNode()
            << " Message Type: " << msg->GetMessageType() << std::endl;
#endif
  ...
  if (msg->GetMessageType() == IdealControlMessage::CQI_FEEDBACKS) {
    ...
    mac->ReceiveCqiIdealControlMessage(cqiMsg);
             ...
             IdealControlMessage::CQI_WITH_MUTE_FEEDBACKS) {
    CqiWithMuteIdealControlMessage *cqi_msg =
