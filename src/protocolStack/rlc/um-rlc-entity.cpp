...
int UmRlcEntity::completed_flows = 0;
...
UmRlcEntity::TransmissionProcedure (int availableBytes)
{
  ...
  if (bearer->GetApplication ()->GetApplicationType () == Application::APPLICATION_TYPE_INFINITE_BUFFER)
    ...
    {
		// double time_now = Simulator::Init()->Now();
		// double time_queued = queue->Peek().GetTimeStamp();
		// double time_in_queue = time_now - time_queued;
		// cout << "Time in Queue: " << time_in_queue << endl;
		// if (time_in_queue < 0.0001){
		// time_in_queue = 0.0001;
		// }
      while (availableBytes > 0 && !queue->IsEmpty ())
      {
          ...
          if ( (packet->GetRLCHeader()->IsAFragment() && packet->GetRLCHeader()->IsTheLatestFragment())
            ...
            if ( tags->GetApplicationType() == PacketTAGs::APPLICATION_TYPE_IPFLOW &&
			  ...
			  if (tags->GetFrameNumber() < 7){
				std::cout << "ipflow end app -"
				<< " flow: " << tags->GetFrameNumber()
				<< " fct: " << flow_complete_time
				<< " flowsize: " << tags->GetApplicationSize()
				<< " priority: " << bearer->GetPriority()
				<< " flow_id: " << bearer->GetDestination()->GetIDNetworkNode()
				<< std::endl;
				completed_flows++;
				std::cout << "Completed flows: " << completed_flows << std::endl;
				if (completed_flows >= 8*4*7){
					cout << "All IP Flows Complete. Exiting\n";
					cerr << "All IP Flows Complete. Exiting\n";
					std::exit(0);
				}
			  }
  ...
  if (p->GetRLCHeader ()->IsAFragment () && p->GetRLCHeader ()->IsTheLatestFragment())
    {
	  ...
	  if (m_incomingPacket.size () == (size_t)numberOfPackets)
	    {
		  RadioBearerSink *bearer = (RadioBearerSink*) GetRadioBearerInstance ();
