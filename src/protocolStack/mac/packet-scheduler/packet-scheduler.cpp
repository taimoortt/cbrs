...
void PacketScheduler::SetMacEntity(MacEntity *mac) { m_mac = mac; }

MacEntity *PacketScheduler::GetMacEntity(void) { return m_mac; }

void PacketScheduler::Schedule(void) { DoSchedule(); }

void PacketScheduler::DoSchedule(void) {}

void PacketScheduler::StopSchedule() { DoStopSchedule(); }
...
FlowsToSchedule *PacketScheduler::GetFlowsToSchedule(void) const {
...
RadioBearer *FlowToSchedule::GetBearer(void) { return m_bearer; }

void FlowToSchedule::SetSpectralEfficiency(std::vector<double> s) {
  m_spectralEfficiency = s;
}

std::vector<double> FlowToSchedule::GetSpectralEfficiency(void) {
  return m_spectralEfficiency;
}

void FlowToSchedule::UpdateAllocatedBits(int allocatedBits) {
...
int FlowToSchedule::GetAllocatedBits(void) const { return m_allocatedBits; }

int FlowToSchedule::GetTransmittedData(void) const { return m_transmittedData; }

void FlowToSchedule::SetDataToTransmit(int dataToTransmit) {
  m_dataToTransmit = dataToTransmit;
}

int FlowToSchedule::GetDataToTransmit(void) const { return m_dataToTransmit; }

std::vector<int> *FlowToSchedule::GetListOfAllocatedRBs() {
  return &m_listOfAllocatedRBs;
}

std::vector<int> *FlowToSchedule::GetListOfSelectedMCS() {
  return &m_listOfSelectedMCS;
}

void FlowToSchedule::SetCqiFeedbacks(std::vector<int> &cqiFeedbacks) {
  m_cqiFeedbacks = cqiFeedbacks;
}

std::vector<int> &FlowToSchedule::GetCqiFeedbacks(void) {
  return m_cqiFeedbacks;
}

void FlowToSchedule::SetCqiWithMuteFeedbacks(
    std::vector<CqiReport> &cqiFeedbacks) {
  m_cqiWithMuteFeedbacks = cqiFeedbacks;
}

std::vector<CqiReport> &FlowToSchedule::GetCqiWithMuteFeedbacks(void) {
  return m_cqiWithMuteFeedbacks;
}

void FlowToSchedule::SetRSRPReport(RSRPReport &rsrp_report) {
  m_rsrp_report = rsrp_report;
}

RSRPReport &FlowToSchedule::GetRSRPReport(void) { return m_rsrp_report; }
