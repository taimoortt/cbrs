...
NVSDownlinkScheduler::~NVSDownlinkScheduler() { Destroy(); }

void NVSDownlinkScheduler::SelectFlowsToSchedule() {
  ...
  for (auto it = bearers->begin(); it != bearers->end(); it++) {
    ...
    if (bearer->HasPackets() &&
      ...
      std::vector<CqiReport> &cqi_withmute_feedbacks =
          ueRecord->GetCQIWithMute();
...
double NVSDownlinkScheduler::ComputeSchedulingMetric(RadioBearer *bearer,
