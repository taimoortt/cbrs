...
DL_FLS_PacketScheduler::~DL_FLS_PacketScheduler() { Destroy(); }

void DL_FLS_PacketScheduler::SetLowerLevelSchedulerType(
    LowerLevelSchedulerType type) {
...
#ifdef SCHEDULER_DEBUG
          std::cout << "\t\t  added packet of bytes " << (*it)->GetSize()
                    << std::endl;
          //(*it)->Print ();
#endif
...
double DL_FLS_PacketScheduler::ComputeSchedulingMetric(
  ...
  if (bearer->GetApplication()->GetApplicationType() ==
    ...
    if (GetLowerLevelSchedulerType() ==
        DL_FLS_PacketScheduler::MAXIMUM_THROUGHPUT) {
      metric = spectralEfficiency * 180000.;
    }

    if (GetLowerLevelSchedulerType() ==
        DL_FLS_PacketScheduler::PROPORTIONAL_FAIR) {
      metric =
  ...
  for (std::vector<RadioBearer *>::iterator it = bearers->begin();
    ...
    if (bearer->GetApplication()->GetApplicationType() ==
...
#ifdef SCHEDULER_DEBUG
        std::cout << "\t dataToTransmit " << dataToTransmit << " minData "
                  << minData << " maxData " << maxData << std::endl;
#endif
      ...
      std::cout << "\t selected flow "
                << (*it)->GetApplication()->GetApplicationID() << std::endl;
      qos->Print();
      std::cout << "\t queue size  " << queueSize << "\n\t DATA TO TRANSMIT "
                << (ceil)(dataToTransmit) << std::endl;
#endif
...
void DL_FLS_PacketScheduler::Select_RT_FlowsToSchedule() {
  ...
  for (std::vector<RadioBearer *>::iterator it = bearers->begin();
    ...
    if (bearer->HasPackets() &&
      ...
      for (int i = 0; i < numberOfCqi; i++) {
        double sEff = GetMacEntity()->GetAmcModule()->GetEfficiencyFromCQI(
            cqiFeedbacks.at(i));
        spectralEfficiency.push_back(sEff);
      }

      // create flow to schedule record
      InsertFlowToSchedule(bearer, dataToTransmit, cqiFeedbacks);
...
void DL_FLS_PacketScheduler::Select_NRT_FlowsToSchedule() {
  ClearFlowsToSchedule();
...
void DL_FLS_PacketScheduler::RBsAllocation() {
#ifdef SCHEDULER_DEBUG
  ...
  for (FlowsToSchedule::iterator it = flows->begin(); it != flows->end();
    ...
    if (flow->GetListOfAllocatedRBs()->size() > 0) {
      ...
      int transportBlockSize =
          amc->GetTBSizeFromMCS(mcs, flow->GetListOfAllocatedRBs()->size());
