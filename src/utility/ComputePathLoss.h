...
static double ComputePathLossForInterference(NetworkNode *src,
  ...
  if (src->GetNodeType() == NetworkNode::TYPE_HOME_BASE_STATION &&
    ...
    /*
        double distance = src->GetMobilityModel ()->GetAbsolutePosition
       ()->GetDistance ( dst->GetMobilityModel ()->GetAbsolutePosition ());

        if( src->GetCell()->GetCellCenterPosition()->GetCoordinateZ() > 0
                                && ((UserEquipment*) dst)->IsIndoor()
                                   &&
       src->GetCell()->GetCellCenterPosition()->GetCoordinateZ() !=
       dst->GetCell()->GetCellCenterPosition()->GetCoordinateZ())
                {
                        int n = (int) abs(
       src->GetCell()->GetCellCenterPosition()->GetCoordinateZ() -
       dst->GetCell()->GetCellCenterPosition()->GetCoordinateZ() );
        ...
        pathLoss = max( minimumCouplingLoss, 127 + ( 30 * log10 (distance *
       0.001) ) + floorPenetration); return pathLoss;
        */
    ...
    double distance =
        src->GetMobilityModel()->GetAbsolutePosition()->GetDistance(
            dst->GetMobilityModel()->GetAbsolutePosition());
    int *nbWalls =
        GetWalls((Femtocell *)(src->GetCell()), (UserEquipment *)dst);
