...
int
NetworkManager::GetNbMacroCell (void)
{
  return m_nbMacroCell;
}
void
NetworkManager::SetNbMacroCell (int nbCell)
{
  m_nbMacroCell = nbCell;
}
...
NetworkManager::GetCellIDFromPosition (CartesianCoordinates *position)
{
  ...
  for (iter = cellContainer->begin ();
    ...
    if (distance <= cell->GetRadius() * 1000)
      {
...
NetworkManager::HandoverProcedure(double time, UserEquipment* ue, NetworkNode* oldTarget, NetworkNode* newTarget)
{
  ...
  // Simulator::Init()->Schedule(Simulator::Init()->Now(),
...
#ifdef HANDOVER_DEBUG
       std::cout << "update cell and new target enb for the ue " << ue->GetIDNetworkNode() << " \t " << oldTargetNode->GetIDNetworkNode() << "->" << ue->GetTargetNode()->GetIDNetworkNode() << std::endl;
#endif
