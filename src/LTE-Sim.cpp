 ...
 * (i) select one of example scenarios developed in Scenario/ ("./LTE-Sim -h"
 * for details) (ii) create a new scenario and add its reference into the main
 * program.
 *
  ...
  if (argc > 1) {
    ...
    if (strcmp(argv[1], "SingleCellWithI") == 0) {
      ...
      if (argc == 15)
        ...
        seed = -1;
      SingleCellWithInterference(nbCells, radius, nbUE, nbVoIP, nbVideo, nbBE,
                                 nbCBR, sched_type, frame_struct, speed,
