...
static void MultiCell(int nbCell, double radius, int nbUE, int nbVoIP,
                      ...
                      double _bandwidth, int reuse_policy, int seed) {
  ...
  if (reuse_policy == 0) { // STATIC PARTITIONING
    ...
    spectrums = RunFrequencyReuseTechniques(nbCell, /*reuse*/ 0, bandwidth);
  } else if (reuse_policy == 1) { // REUSE
    cout << "Freq Reuse Policy: Reuse" << endl;
    spectrums = RunFrequencyReuseTechniques(nbCell, /*reuse*/ 1, bandwidth);
  } else if (reuse_policy == 2) { // FERMI
    cout << "Freq Reuse Policy: Fermi" << endl;
  ...
  } else if (reuse_policy == 3) { // FCBRS
    cout << "Freq Reuse Policy: FCBRS" << endl;
    std::map<int, int> ues_per_cell = GetUEsPerCell(*positions, nm);
    spectrums = DivideResourcesFCBRS(nbCell, bandwidth, ues_per_cell);
  } else if (reuse_policy == 4) { // IDEAL
    cout << "Freq Reuse Policy: IDEAL" << endl;
    std::map<int, int> interference_impacted_users =
        GetInterferenceLimitedUsersPerCell(*positions, nm);
    std::map<int, int> ues_per_cell = GetUEsPerCell(*positions, nm);
  ...
  while (mobility_array.size() < total_ues)
    mobility_array.push_back(speeds[0]); // Fill with the first speed if needed
  ...
  // // CRITICAL: Update all channel realizations after bandwidth manager
  // changes
  ...
  //     PropagationLossModel *propLossModel =
  //         dlChannel->GetPropagationLossModel();
  ...
  //           propLossModel->GetChannelRealization(enb, ue);
  ...
  //     PropagationLossModel *propLossModel =
  //         ulChannel->GetPropagationLossModel();
  ...
  //       // Force update of channel realization for this UE-eNB pair
  //       ChannelRealization *channelRealization =
  //           propLossModel->GetChannelRealization(ue, enb);
  //       if (channelRealization != NULL) {
  //         // Force update by calling UpdateModels directly
  //         channelRealization->UpdateModels();
  //       }
  ...
  for (int i = 0; i < eNBs->size(); i++) {
    ...
    std::vector<ENodeB::UserEquipmentRecord *> ue_records =
        *enb->GetUserEquipmentRecords();
    cout << "Cell: " << enb->GetIDNetworkNode() << " has " << ue_records.size()
         << " UEs" << endl;
      ...
      if (ue->GetSliceID() == i) {
        ...
        if (app == 'v') {
          ...
          ClassifierParameters *cp = new ClassifierParameters(
              ...
              destinationPort, TransportProtocol::TRANSPORT_PROTOCOL_TYPE_UDP);
