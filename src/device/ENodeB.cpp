...
void ENodeB::UserEquipmentRecord::SetUE(UserEquipment *UE) { m_UE = UE; }

UserEquipment *ENodeB::UserEquipmentRecord::GetUE(void) const { return m_UE; }

void ENodeB::UserEquipmentRecord::SetCQI(std::vector<int> cqi) {
  // cout << "Set CQI called" << endl;
  // for (size_t i = 0; i < cqi.size(); i++) {
  //   cout << "CQI for RB " << i << " : " << cqi.at(i) << endl;
  // }
  m_cqiFeedback = cqi;
}
...
void ENodeB::UserEquipmentRecord::SetCQIWithMute(std::vector<CqiReport> cqi) {
  m_cqiWithMuteFeedback = cqi;
}

std::vector<CqiReport> &ENodeB::UserEquipmentRecord::GetCQIWithMute(void) {
  return m_cqiWithMuteFeedback;
}

void ENodeB::UserEquipmentRecord::SetRSRP(RSRPReport report) {
  m_rsrp_report = report;
}

RSRPReport &ENodeB::UserEquipmentRecord::GetRSRP(void) { return m_rsrp_report; }
...
void ENodeB::UserEquipmentRecord::SetUlMcs(int mcs) { m_ulMcs = mcs; }

int ENodeB::UserEquipmentRecord::GetUlMcs(void) { return m_ulMcs; }
