...
NetworkNode *UserEquipment::GetTargetNode(void) { return m_targetNode; }
...
void UserEquipment::SetCqiManager(CqiManager *cm) { m_cqiManager = cm; }

CqiManager *UserEquipment::GetCqiManager(void) { return m_cqiManager; }

void UserEquipment::SetIndoorFlag(bool flag) { m_isIndoor = flag; }

bool UserEquipment::IsIndoor(void) { return m_isIndoor; }

void UserEquipment::CalculateUpdatedRSRP() {
  // cout << "Updating RSRP for UE: " << this->GetIDNetworkNode() << endl;
  double powerTx =
      pow(10., (this->GetTargetNode()->GetPhy()->GetTxPower() - 30) /
                   10); // in natural unit
  int channels = this->GetTargetNode()
                     ->GetPhy()
                     ->GetBandwidthManager()
                     ->GetDlSubChannels()
                     .size();
  ...
  double distance =
      this->GetMobilityModel()->GetAbsolutePosition()->GetDistance(
          this->GetTargetNode()->GetMobilityModel()->GetAbsolutePosition());
  double m_pathLoss =
      128.1 + (37.6 * log10(distance * 0.001)); // URBAN AREA PATH LOSS
  ...
  interference_rsrp =
      this->GetPhy()->GetInterference()->ComputeInterference(this);
  for (auto it = interference_rsrp.begin(); it != interference_rsrp.end();
       ++it) {
  ...
  //   cout << "Interfering ID: " << pair.first << "\tPower: " << pair.second <<
  //   endl;
  // }
...
double UserEquipment::GetPrimaryRSRP() { return primary_rsrp; }
...
std::vector<std::vector<float>> UserEquipment::GenOneTrace(int rsrp) {
  ...
  const int num_rbg = 65;
  std::vector<std::vector<float>> rsrps;
  rsrps.reserve(1000); // Assuming a typical file contains 1000 TTI entries
  std::string line;
  ...
  while (getline(file, line)) {
    ...
    float num;
    int count = 0;
    std::vector<float> current_rsrps;
    current_rsrps.reserve(num_rbg);

    while (iss >> num && count < num_rbg) {
      if (num != -1) {
        current_rsrps.push_back(num);
        count++;
      }
    }
    if (!current_rsrps.empty()) {
      rsrps.push_back(
          std::move(current_rsrps)); // Use move semantics to avoid copying
    }
  }
  return rsrps;
}

void UserEquipment::ReadAllTraces() {
...
map<int, vector<double>> UserEquipment::GetInterferenceTrace(int timestamp) {
  ...
  for (const auto &pair : m_interferenceTrace) {
    const auto &rsrp_tti =
        m_interferenceTrace[pair.first].at(timestamp % TRACE_LENGTH);
    vector<double> trace;
    for (size_t rbg_id = 0; rbg_id != rsrp_tti.size(); rbg_id++) {
      for (int i = 0; i < RBG_SIZE; i++) {
...
void UserEquipment::SetNbCells(int cells) { nbCells = cells; }

int UserEquipment::GetNbCells(void) { return nbCells; }
